...
from util import get_ck_client, get_ck_conn_info

insert_size = 50000


def get_github_id_tz_region_map(conn_info):
    conn_info =  get_ck_conn_info("jump_10090")
...
"""
create table if not exists github_id_main_tz_map_local
...
) engine = ReplicatedMergeTree('/clickhouse/tables/{shard}/github_id_main_tz_map',
           '{replica}') ORDER BY (github_id) SETTINGS index_granularity = 8192;



create table if not exists github_id_main_tz_map on cluster replicated as default.github_id_main_tz_map_local engine =
            Distributed('replicated', 'default', 'github_id_main_tz_map_local', update_at_timestamp);
"""



# get_github_id_tz_region_map("")
